  ...
  private void validateOAuth2Credentials(FailureCollector collector) {
    if (!containsMacro(PROPERTY_CLIENT_ID) && !containsMacro(PROPERTY_CLIENT_SECRET) &&
      ...
      try (CloseableHttpClient closeableHttpClient = httpclientBuilder.build()) {
        ...
        String errorMessage = String.format("Failed to validate OAuth for the request with message: %s",
          e.getMessage());
        collector.addFailure(errorMessage, null);
      }
    }
    ...
    try (CloseableHttpResponse response = httpClient.executeHTTP(getUrl())) {
      ...
      if (statusCode != HttpStatus.SC_OK) {
        ...
        if (entity != null) {
          ...
          String errorMessage =
            String.format("Credential validation request failed with status code: '%d', with response: '%s'",
              statusCode, errorResponse);
          collector.addFailure(errorMessage, "Please ensure that correct credentials are provided.");
        }
      }
    } catch (Exception e) {
      String errorMessage = String.format("Error validating credentials for request with message: %s ", e.getMessage());
      collector.addFailure(errorMessage, "Please ensure that correct credentials are provided.");
    }
  }
